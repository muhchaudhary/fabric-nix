...
class Overview(Window):
    def __init__(
        ...
        self.workspacescroll2 = EventBox(style="background-color: black;",events="smooth-scroll",)
    ...
    def on_test_scroll(self, widget, event):
        if event.delta_x != 0:
            ...
            if self.scroll_value == 0:
                ...
                self.scroll_value = 0 if event.delta_x > 0 else self.scroll_value + event.delta_x
            else:
                self.scroll_value = 0 if event.delta_x < 0 else self.scroll_value + event.delta_x
